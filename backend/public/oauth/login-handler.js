import { getLoginSession, updateLoginSession, } from './login-store.js';
import { loginToNixaerp, NixaerpAuthError, } from './nixaerp-client.js';
...
async function readBody(req) {
    return new Promise((resolve, reject) => {
        ...
        req.on('data', (chunk) => {
            body += chunk;
            if (Buffer.byteLength(body, 'utf8') > FORM_MAX_SIZE) {
                finish(() => {
...
function redirectToAuthorize(res, sessionId, code, state) {
    /*
...
export async function handleLogin(req, res) {
    ...
    try {
        ...
        const submittedSessionId = params.get('session') ?? '';
        ...
        const authentication = await loginToNixaerp(email, password);
        const updated = updateLoginSession(session.id, {
            userId: authentication.user.id,
            user: authentication.user,
            nixaerpAccessToken: authentication.accessToken,
        });
        if (!updated) {
            sendHtml(res, 400, html(`
          <div class="card">
        ...
        redirectToAuthorize(res, session.id, code, session.state);
    }
    catch (error) {
        console.error('[OAuth] NixaERP login failed:', error);
        if (error instanceof
            NixaerpAuthError) {
            const message = error.status === 401
                ? 'Invalid NixaERP email or password.'
                : error.message;
            sendHtml(res, error.status === 401
                ? 401
                : 502, html(`
          <div class="card">
            <h1>Login Failed</h1>
