...
function isSegmentedLabeledOptionWithIcon(option) {
  return (0, _is.isPlainObject)(option) && !!option?.icon;
}
const InternalSegmented = /*#__PURE__*/React.forwardRef((props, ref) => {
  const defaultName = (0, _util.useId)();
  const {
  ...
  const extendedOptions = React.useMemo(() => options.map(option => {
    if (isSegmentedLabeledOptionWithIcon(option)) {
      ...
      return {
        ...
        }, icon), (0, _is.isReactRenderable)(label) && /*#__PURE__*/React.createElement("span", null, label)))
      };
    }
    return option;
  ...
  const itemRender = (node, {
    ...
    const tooltipProps = (0, _is.isPlainObject)(item.tooltip) ? item.tooltip : {
      title: item.tooltip
    };
