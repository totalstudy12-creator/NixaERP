...
> = {
  ...
  voice:         { icon: '🎙️', color: 'bg-violet-100 text-violet-700' },
};
...
const SkeletonTable = () => (
  <div className="animate-pulse space-y-4 rounded-xl border bg-white p-6 shadow-sm">
...
export function SettingsPage() {
  ...
  const [mcpRevokingAll, setMcpRevokingAll] = useState(false);
  const [mcpName, setMcpName] = useState('ChatGPT');
  const [mcpExpiresDays, setMcpExpiresDays] = useState('30');
  const [mcpStatus, setMcpStatus] = useState<McpStatus | null>(null);
  const [mcpStatusLoading, setMcpStatusLoading] = useState(false);
  const [createdMcpToken, setCreatedMcpToken] = useState<McpTokenCreateResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const timers = [
      window.setTimeout(() => setAuthToken(readToken()), 300),
      window.setTimeout(() => setAuthToken(readToken()), 1200),
      window.setTimeout(() => setAuthToken(readToken()), 2500),
    ];
  ...
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(BILL_EXTRACT_STORAGE_KEY);
      if (!raw) return;
  ...
  const loadSettings = useCallback(
    async () => {
      ...
      try {
        ...
        rawData.forEach((item: SettingItem) => {
          if (
            item &&
            typeof item.key === 'string' &&
            Object.prototype.hasOwnProperty.call(mergedVoiceConfig, item.key)
          ) {
            mergedVoiceConfig[item.key as keyof typeof mergedVoiceConfig] = String(
              item.value ?? mergedVoiceConfig[item.key as keyof typeof mergedVoiceConfig]
            );
          }
        });
        ...
        rawData.forEach((item: SettingItem) => {
          if (
            item &&
            typeof item.key === 'string' &&
            Object.prototype.hasOwnProperty.call(mergedPrinterSettings, item.key)
          ) {
            mergedPrinterSettings[item.key as keyof typeof mergedPrinterSettings] = String(
              item.value ?? mergedPrinterSettings[item.key as keyof typeof mergedPrinterSettings]
            );
          }
        });
        ...
        if (
          mergedPrinterSettings.printer_is_connected === 'true' &&
          mergedPrinterSettings.printer_bluetooth_device_id
        ) {
          setConnectedDevice({
        ...
        setExpandedGroups((previous) => {
          ...
          return { ...previous, [firstGroup]: true };
        });
      } catch (err: unknown) {
        const errorObject = err as { backendMessage?: string; message?: string };
        const message =
  ...
  const loadMcpTokens = useCallback(
    async () => {
      ...
      try {
        ...
        setMcpTokens(
          rawData.map((token: McpToken) => ({
            id: Number(token.id),
            name: String(token.name ?? ''),
            abilities: Array.isArray(token.abilities)
              ? token.abilities.filter((ability): ability is string => typeof ability === 'string')
              : [],
        ...
        showError('MCP token loading failed', message);
      } finally {
        setMcpLoading(false);
      }
    },
  ...
  const checkMcpStatus = useCallback(
    async () => {
      ...
      try {
        ...
        setMcpStatus(null);
        showError('MCP status failed', message);
      } finally {
  ...
  const generateMcpToken = useCallback(
    async () => {
      ...
      const expiresDays = Number(mcpExpiresDays);
      if (!Number.isInteger(expiresDays) || expiresDays < 1 || expiresDays > 90) {
        showError('Invalid expiry', 'MCP token expiry must be between 1 and 90 days.');
        return;
      }

      setMcpGenerating(true);
      try {
        const response = await apiClient.request('POST', '/mcp/tokens', {
          name: trimmedName,
        ...
        if (
          ...
          throw new Error('The server did not return a valid MCP token.');
        }
        ...
        const createdToken: McpTokenCreateResponse = {
          ...
          id: Number(data.id),
          name: String(data.name ?? `NixaERP MCP:${trimmedName}`),
          abilities: Array.isArray(data.abilities) ? data.abilities : ['mcp:read'],
        ...
        await loadMcpTokens();
        showSuccess('MCP token created', 'The read-only MCP token was created successfully.');
      } catch (err: unknown) {
        const errorObject = err as { backendMessage?: string; message?: string };
        const message =
          errorObject?.backendMessage ||
        ...
        showError('MCP token creation failed', message);
      } finally {
  ...
  const revokeMcpToken = useCallback(
    async (token: McpToken) => {
      ...
      setMcpRevokingId(token.id);
      try {
        await apiClient.request('DELETE', `/mcp/tokens/${token.id}`);
        await loadMcpTokens();
        showSuccess('MCP token revoked', `${token.name} has been revoked.`);
      } catch (err: unknown) {
        const errorObject = err as { backendMessage?: string; message?: string };
        const message =
          errorObject?.backendMessage ||
        ...
        showError('MCP revoke failed', message);
      } finally {
  ...
  const revokeAllMcpTokens = useCallback(async () => {
    ...
    try {
      await apiClient.request('DELETE', '/mcp/tokens');
      setMcpTokens([]);
      showSuccess('MCP tokens revoked', 'All MCP tokens have been revoked.');
    } catch (err: unknown) {
      const errorObject = err as { backendMessage?: string; message?: string };
      const message =
        errorObject?.backendMessage ||
  ...
  const copyMcpToken = useCallback(async () => {
    if (!createdMcpToken?.token) return;
    try {
      await navigator.clipboard.writeText(createdMcpToken.token);
      setCopiedKey('mcp-created-token');
      window.setTimeout(() => setCopiedKey(null), 2000);
      showSuccess('Copied', 'MCP token copied to clipboard.');
    } catch {
      showError('Copy failed', 'Unable to copy MCP token.');
    }
  }, [createdMcpToken, showError, showSuccess]);
  ...
  const saveAiConfig = useCallback(() => {
    ...
    if (!trimmedKey) {
      showError('Missing API key', 'Enter a Gemini API key before saving.');
      return;
    }
    if (trimmedKey.length < 20) {
      showError('Invalid API key', 'That key looks too short. Double-check the value.');
      return;
    }
  ...
  const groupedSettings = useMemo(() => {
    ...
    return filtered.reduce<Record<string, SettingItem[]>>((accumulator, item) => {
  ...
  const visibleSettings = Object.values(groupedSettings).reduce(
  ...
  const saveSetting = useCallback(
    async (setting: SettingItem) => {
      ...
      try {
        ...
        await apiClient.request('PUT', `/settings/${encodeURIComponent(setting.key)}`, {
          value: safeValue,
          group: setting.group,
        ...
        const errorObject = err as { backendMessage?: string; message?: string };
        const message =
          errorObject?.backendMessage || errorObject?.message || 'Save failed.';
        showError('Save failed', message);
        setSettings((current) =>
          current.map((item) =>
            item.id === setting.id
              ? { ...item, value: String(item.defaultValue ?? '') }
              : item
          )
        );
      } finally {
    ...
    for (const setting of changed) {
      ...
      try {
        ...
        showError(
          ...
          errorObject?.backendMessage || errorObject?.message || 'Error'
        );
      } finally {
        setSaving((previous) => ({ ...previous, [setting.key]: false }));
      }
  ...
  const createSetting = useCallback(async () => {
    ...
    setSaving((previous) => ({ ...previous, new: true }));
    try {
      await apiClient.request('POST', '/settings', { ...newSetting, key });
      showSuccess('Created', `Setting ${key} added.`);
      ...
      showError(
        'Create failed',
        errorObject?.backendMessage || errorObject?.message || 'Failed to create setting.'
      );
    } finally {
  ...
  const deleteSetting = useCallback(
    async (key: string) => {
      ...
      if (!confirmed) return;
      try {
        await apiClient.request('DELETE', `/settings/${encodeURIComponent(key)}`);
  ...
  const saveVoiceSettings = useCallback(async () => {
    ...
    try {
      setSaving((previous) => ({ ...previous, voice: true }));
      ...
      showError(
        ...
        errorObject?.backendMessage || errorObject?.message || 'Unable to save voice settings.'
      );
    } finally {
  ...
  const savePrinterSettings = useCallback(async () => {
    ...
    try {
      for (const [key, value] of Object.entries(printerSettings)) {
        await apiClient.request('PUT', `/settings/${encodeURIComponent(key)}`, {
          value,
          group: 'printer',
      ...
      showError(
        ...
        errorObject?.backendMessage || errorObject?.message || 'Unable to save printer settings.'
      );
    } finally {
  ...
  const scanBluetoothPrinters = useCallback(async () => {
    ...
    try {
      ...
      showSuccess(
        ...
        `Selected: ${device.name || 'Unnamed printer'}`
      );
    } catch (err: unknown) {
      ...
      if (errorObject?.name === 'NotFoundError') {
        ...
        showError(
          ...
          errorObject?.message || 'Unable to find printers.'
        );
      }
    } finally {
  ...
  const connectBluetoothDevice = useCallback(
    async (device: BluetoothDevice) => {
      ...
      try {
        ...
        showError('Connection failed', errorObject?.message || 'Could not connect.');
      } finally {
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className={`rounded-lg px-2 py-1 text-lg ${meta.color}`}>
                          {meta.icon}
                        </span>
                              ...
                              <div className="mb-3 flex items-start justify-between gap-2">
                                ...
                                <div className="flex gap-1">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      void copyToClipboard(setting.key, setting.value)
                                    }
          ...
          <div className="rounded-2xl border border-cyan-200 bg-gradient-to-r from-cyan-50 via-white to-blue-50 p-5 shadow-sm">
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              <div>
                ...
                <h2 className="mt-1 text-xl font-bold text-slate-800">
                  Default Printer &amp; Bluetooth Connection
                </h2>
                <p className="text-sm text-slate-600">
            ...
            <div className="mt-6 rounded-xl border border-slate-200 bg-white p-5">
              <h3 className="mb-4 flex items-center gap-2 font-semibold text-slate-800">
                <FiBluetooth className="text-cyan-600" size={18} />
                Bluetooth Printer
              </h3>
              ...
              {connectedDevice && (
                <div className="mt-4 flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3">
                  ...
                  <span className="text-sm">
                    Connected to{' '}
                    <strong>{connectedDevice.name || 'printer'}</strong>
                  </span>
                </div>
              )}
              ...
              {!('bluetooth' in navigator) && (
                <p className="mt-3 text-xs text-amber-600">
                  ⚠️ Web Bluetooth is not supported in this browser. Use a
      ...
      {activeTab === 'ai' && (
        <div className="space-y-6">
            ...
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              <div>
                ...
                <h2 className="mt-1 flex items-center gap-2 text-xl font-bold text-slate-800">
                  <FiCpu className="text-violet-600" size={20} />
                  AI API Key &amp; Model
                </h2>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl border border-violet-200 bg-white px-4 py-2 text-sm font-medium text-violet-700 hover:bg-violet-50 disabled:opacity-60"
                >
                  {aiTesting ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <label className="text-sm text-slate-700 md:col-span-2">
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowAiKey((previous) => !previous)}
                  ...
                  >
                    {showAiKey ? <FiEyeOff size={14} /> : <FiEye size={14} />}
                  </button>
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  Get a free key from{' '}
            ...
            {aiTestResult && (
              ...
              >
                ...
                <span className="break-words">{aiTestResult.message}</span>
              </div>
            )}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <FiZap className="mb-2 text-violet-600" />
              <h4 className="font-semibold text-slate-800">Shared with AI scanner</h4>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                The Purchase page reads these settings automatically when you click
                "Scan with AI".
              </p>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <h4 className="font-semibold text-slate-800">Rotate often</h4>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Treat the API key like a password. Revoke and regenerate it in Google AI
                Studio if it ever leaks.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="px-6 py-4">
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => void copyToClipboard('api-url', fullApiUrl)}
          ...
          <div className="overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-sm">
              ...
              <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                <div>
                  ...
                  <p className="max-w-3xl text-sm text-slate-600">
                    Create dedicated read-only MCP credentials for ChatGPT and other MCP
                    clients.
                  </p>
                </div>
            ...
            <div className="space-y-6 p-6">
              ...
              <div className="grid gap-3 md:grid-cols-4">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold text-slate-800">
                    {mcpStatus?.enabled ? 'Enabled' : 'Not checked'}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold text-slate-800">
                    {mcpStatus?.authenticated ? 'Authenticated' : 'Not checked'}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
                ...
                <div>
                  <p className="font-semibold text-amber-800">Read-only security</p>
                  <p className="mt-1 text-sm text-amber-700">
                    MCP tokens created here receive only{' '}
                    <code className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-xs">
                      mcp:read
                    </code>
                    . They cannot create, update, delete, make payments, or directly access
                    the database.
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-blue-800">Current MCP API</p>
                    <div className="mt-2 space-y-1 font-mono text-xs text-blue-700">
                      <div>Status: {fullApiUrl}/mcp/status</div>
                      <div>Context: {fullApiUrl}/mcp/context</div>
                      <div>Tokens: {fullApiUrl}/mcp/tokens</div>
                    </div>
                    <p className="mt-2 text-xs text-blue-700">
                      These are the Laravel MCP authentication endpoints. The remote MCP
                      protocol endpoint will be connected after the TypeScript MCP server
                      is deployed.
                    </p>
                  </div>
                </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-xl border border-slate-200 bg-white p-4">
                  <FiShield className="mb-2 text-emerald-600" />
                  <h4 className="font-semibold text-slate-800">User scoped</h4>
                  <p className="mt-1 text-xs leading-5 text-slate-500">
                    Every MCP token belongs to the currently authenticated ERP user.
                  </p>
                </div>

                <div className="rounded-xl border border-slate-200 bg-white p-4">
                  <FiDatabase className="mb-2 text-blue-600" />
                  <h4 className="font-semibold text-slate-800">No database access</h4>
                  <p className="mt-1 text-xs leading-5 text-slate-500">
                    MCP communicates through Laravel APIs instead of directly accessing
                    MySQL.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-4">
                  ...
                  <h4 className="font-semibold text-slate-800">Short-lived token</h4>
                  <p className="mt-1 text-xs leading-5 text-slate-500">
                    Tokens are limited to a maximum 90-day lifetime.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 px-6 py-4">
              <div className="flex items-center gap-2">
                <code className="min-w-0 flex-1 break-all rounded-lg bg-slate-100 px-3 py-3 text-sm font-mono">
                  {!authToken
                    ...
                    : showAuthToken
                      ? authToken
                      : '•'.repeat(Math.min(authToken.length, 40))}
                </code>

                <button
                  type="button"
                  onClick={() => setShowAuthToken((previous) => !previous)}
                  disabled={!authToken}
                ...
                <button
                  type="button"
                  onClick={() =>
                    authToken && void copyToClipboard('auth-token', authToken)
                  }
                  disabled={!authToken}
                  className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"
                  title="Copy token"
                >
                  {copiedKey === 'auth-token' ? (
                    <FiCheck className="text-emerald-500" size={16} />
                  ) : (
                    <FiCopy size={16} />
                  )}
              ...
              <div className="flex gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
                ...
                <p className="text-xs text-amber-700">
                  This is your normal ERP authentication credential. Do not use the normal
                  ERP token as the MCP credential. Use the dedicated MCP token above.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3 px-6 py-4 sm:grid-cols-2">
              ...
              >
                ...
                API Status
                <FiExternalLink className="ml-auto" size={14} />
              </a>
              ...
              <button
                type="button"
                onClick={() =>
                  void copyToClipboard('mcp-context-url', `${fullApiUrl}/mcp/context`)
                }
              ...
              >
                ...
                {copiedKey === 'mcp-context-url' ? (
                  <FiCheck className="ml-auto text-emerald-500" size={14} />
                ) : (
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setNewSetting((previous) => ({ ...previous, key: event.target.value }))
                  }
                  maxLength={100}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setNewSetting((previous) => ({ ...previous, value: event.target.value }))
                  }
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setNewSetting((previous) => ({ ...previous, group: event.target.value }))
                  }
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                >
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-emerald-800">
                    Copy this token now. For security, it is not stored in the browser or
                    shown again after this window is closed.
                  </p>
                </div>
            ...
            <div className="space-y-5 p-6">
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowMcpToken((previous) => !previous)}
                    className="rounded-lg border border-slate-200 p-3 text-slate-600 hover:bg-slate-50"
                    title={showMcpToken ? 'Hide token' : 'Show token'}
                  >
                    {showMcpToken ? <FiEyeOff size={17} /> : <FiEye size={17} />}
                  </button>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <p className="mt-1 text-sm font-medium text-emerald-700">mcp:read</p>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <p className="mt-1 text-sm font-medium text-slate-800">
                    {createdMcpToken.expires_at
                      ? new Date(createdMcpToken.expires_at).toLocaleString()
                      : 'Unknown'}
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-amber-800">Keep this token private</p>
                    <p className="mt-1 text-xs leading-5 text-amber-700">
                      Never commit it to Git, put it in frontend source code, share it in
                      screenshots, or include it in URLs.
                    </p>
                  </div>
                </div>
              </div>
